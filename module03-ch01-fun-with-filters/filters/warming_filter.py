...
class WarmingFilter:
    """Makes image look warmer: more red, less blue, higher saturation."""
    ...
    def render(self, img_rgb):
        ...
        # Warm the image: increase red, decrease blue
        c_r = cv2.LUT(c_r, self.incr_ch_lut).astype(np.uint8)
        c_b = cv2.LUT(c_b, self.decr_ch_lut).astype(np.uint8)
        img_rgb = cv2.merge((c_r, c_g, c_b))

        # Increase color saturation (convert to HSV, boost S channel)
        c_h, c_s, c_v = cv2.split(cv2.cvtColor(img_rgb, cv2.COLOR_RGB2HSV))
        c_s = cv2.LUT(c_s, self.incr_ch_lut).astype(np.uint8)

        return cv2.cvtColor(cv2.merge((c_h, c_s, c_v)), cv2.COLOR_HSV2RGB)

    def save_result(self, output_image, filename="warming_output.jpg"):
        ...
        print(f"Saved warming filter result to: {filename}")